...
class CleanSettings_Batch {
    ...
    function clean_default_mailbody() {
        ...
        if (!$match) {
            $diffs[] = __FILE__ . ":" . __LINE__ . ":\n" . $dmp->line_diff_toUnified($dmp->line_diff($null_mailer->expand($t0, "body"), $xmb));
            $t0 = str_replace("On behalf of the %CONFNAME% adjudication committee, %REQUESTERCONTACT% has asked you to review %CONFNAME% submission #%NUMBER%.\n%IF(REASON)%\nThey supplied this note: %REASON%\n%ENDIF%\n",
                "On behalf of the %CONFNAME% adjudication committee, %REQUESTERCONTACT% has asked you to review %CONFNAME% submission #%NUMBER%.%IF(REASON)%\n\nThey supplied this note: %REASON%%ENDIF%\n\n", $t0);
