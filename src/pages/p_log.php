...
class Log_Page {
    ...
    function print_page($leg, $page) {
        ...
        if (!$this->viewer->privChair || !empty($this->exclude_pids)) {
            ...
            if (!$this->viewer->privChair) {
                $conf->feedback_msg(MessageItem::marked_note("<0>Only showing your actions, plus entries for applications you administer"));
                ...
                if ($this->qreq->forceShow) { // XXX never true
                    ...
                    $conf->feedback_msg(MessageItem::marked_note("<5>Not showing entries for " . $conf->hotlink("conflicted administered applications", "search", ["q" => join(" ", array_keys($this->exclude_pids))])));
                }
            }
    ...
    private function print_entry($row, $n, $leg) {
        ...
        if (($pids = $leg->paper_ids($row))) {
            if (count($pids) === 1)
                $at .= ' (' . $conf->hotlink("application " . $pids[0], "paper", ["p" => $pids[0]], ["class" => "track"]) . ')';
            else {
                $at .= ' (' . $conf->hotlink("applications", "search", ["t" => "all", "q" => join(" ", $pids)]);
                foreach ($pids as $i => $p) {
