        ...
        if (!$this->viewer->privChair || !empty($this->exclude_pids)) {
            ...
            if (!$this->viewer->privChair) {
                ...
                if ($this->qreq->forceShow) { // XXX never true
                    ...
                    $conf->feedback_msg(MessageItem::marked_note("<5>Not showing entries for " . Ht::link("conflicted administered applications", $conf->hoturl("search", "q=" . join("+", array_keys($this->exclude_pids))))));
                }
            }
            echo '</div>';
        }
