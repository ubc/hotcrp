...
class Home_Page {
    ...
    function print_search(Contact $user, Qrequest $qreq, ComponentSet $gx) {
        ...
        echo '<div class="homegrp d-table" id="homelist">',
            ...
            $this->conf->hotform("search", null, ["method" => "get", "class" => "form-basic-search", "role" => "search"]),
    ...
    function print_reviews(Contact $user, Qrequest $qreq, ComponentSet $gx) {
        ...
        if ($user->isPC && $user->can_review_any()) {
            echo '  <span class="hint">As a PC member, you may review ', $conf->hotlink("any submitted application", "search", ["q" => "", "t" => "s"]), ".</span><br>\n";
        } else if ($user->privChair) {
            echo '  <span class="hint">As an administrator, you may review ', $conf->hotlink("any submitted application", "search", ["q" => "", "t" => "s"]), ".</span><br>\n";
        }
