...
class AdminHome_Page {
    ...
    static function print(Contact $user) {
        ...
        if ($conf->opt("shortNameDefaulted")) {
            $ml[] = MessageItem::warning_note("<5><a href=\"" . $conf->hoturl("settings", "group=basics") . "\">Set the program abbreviation</a> to a short name for your program, such as “OSDI ’14”");
        } else if (simplify_whitespace($conf->short_name) != $conf->short_name) {
            $ml[] = MessageItem::warning("<5>The <a href=\"" . $conf->hoturl("settings", "group=basics") . "\">program abbreviation</a> setting has a funny value. To fix it, remove leading and trailing spaces, use only space characters (no tabs or newlines), and make sure words are separated by single spaces (never two or more)");
        }
        // Site contact
        $site_contact = $conf->site_contact();
        if (!$site_contact->email || $site_contact->email == "you@example.com") {
			$ml[] = MessageItem::urgent_note("<5><a href=\"" . $conf->hoturl("settings", "group=basics") . "\">Set the program contact’s name and email</a> so submitters can reach someone if things go wrong");
        }
        ...
        if (($row = $result->fetch_row())) {
            $ml[] = MessageItem::marked_note('<5>PC members have indicated application conflicts (using review preferences of &#8722;100 or less) that aren’t yet confirmed. <a href="' . $conf->hoturl("=conflictassign") . '" class="nw">Confirm these conflicts</a>');
        }
        // Weird URLs?
