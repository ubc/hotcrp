...
class Offline_Page {
    ...
    function print() {
        ...
        echo '<li>', $conf->hotlink("Blank form", "offline", ["download" => 1]), '</li>',
            '</ul>
<div class="f-d"><strong>Tip:</strong> Use ', $conf->hotlink("Search", "search", ["q" => ""]), ' &gt; Download to choose individual applications.</div>',
            "</fieldset>";
        ...
        if ($conf->setting("tag_rank")) {
            ...
            if ($this->user->isPC) {
                echo "<li>", $conf->hotlink("All submitted applications", "search", ["fn" => "get", "getfn" => "rank", "tag" => "~{$ranktag}", "q" => "", "t" => "s", "p" => "all"]), "</li>";
            }
            echo '</ul></fieldset>', "\n";
