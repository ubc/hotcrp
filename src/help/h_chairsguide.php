...
form also can include:</p>
  ...
  tracks include external review committees, PC-app review committees, and
  multi-track programs.</li>\n";

        } else if ($gj->itemid === 6) {
            echo "<li><p><strong>", $hth->hotlink("Collect review preferences (bids) from the PC.", "reviewprefs"), "</strong>
  PC members can enter per-application review preferences (also known as bids) to
  ...
  ", which allows the PC to see abstracts for registered applications that haven’t yet
  been submitted.</p></li>\n";
...
program system, including reviewer identities for conflicted applications.
...
<p>First, each application can be assigned a <em>application administrator</em>: a PC member
...
application, and can, for example, use the autoassignment tool, but they cannot change
program settings. When a application
has an administrator, chair conflicts cannot be overridden.</p>

<p>Application administrators make life easy for PC reviewers and greatly restrict
...
administrator’s identity.</p>\n\n";
  ...
  (During most programs’ review periods, a PC member can see a application’s reviews
  only after completing their own review for that application.)</p></li>\n";

        } else if ($gj->itemid === 3) {
            echo "<li><p><strong>", $hth->search_link("Examine application scores", "show:scores"),
  ...
  find all applications with at least two overall merit ratings of 2 or better.</p></li>\n";
  ...
  rejection, such as “accept as short application.”</p></li>\n";
...
themselves.</p></li>\n";
            ...
            echo "<li><p><strong>", $hth->setting_link("Collect final applications (optional).", "final_open"),
